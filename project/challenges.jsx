// Design Challenge page — current/active challenge, history, leaderboard, designer roster

const { useState: useStateCH, useEffect: useEffectCH } = React;

function useCountdown(deadline) {
  const [now, setNow] = useStateCH(Date.now());
  useEffectCH(() => {
...
function ActiveChallengeCard({ challenge }) {
  ...
  // Big crammed-together display title (mimicking the no-spaces hero on the source)
...
function StatsStrip({ data }) {
  const totalChallenges = 3 + 1; // 3 previous + 1 active
...
function ChallengesPage() {
  const data = window.GPX_DATA.CHALLENGES;
  return (
...
Object.assign(window, { ChallengesPage });
