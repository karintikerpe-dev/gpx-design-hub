...
const useStateAP = React.useState;
const useEffectAP = React.useEffect;
const useRefAP = React.useRef;

function ArticlePage({ article, onBack, onUpdate, onDelete }) {
  const { user } = useAuth();
  const [editing, setEditing] = useStateAP(!article.body && article.author === (user?.name || ""));
  ...
  const coverFileRef = useRefAP(null);

  // sync if a different article is loaded
  useEffectAP(() => {
    setDraft({
      title: article.title,
      dek: article.dek,
...
// Light markdown-ish renderer: ## h2, ### h3, > blockquote, - list, blank line = paragraph break
// Image refs: ![caption](img:N) renders as <figure> from images[N]
function RenderProse({ text, images = [] }) {
  ...
  for (const raw of lines) {
    const line = raw.replace(/^[\s\t]+/, ""); // strip leading whitespace/tabs (handles pasted bullets like "\t•\t...")
    ...
    // Inline-bullet line: "- one - two - three" → split into items
    if (/^[-*•]\s.+\s[-*•]\s/.test(line.trim())) {
      flush();
      const items = line.trim().split(/\s+[-*•]\s+/).filter(Boolean);
      // first item still has leading "- "
...
// inline: **bold**, *italic*, `code`, [label](url), bare URLs, mailto
function inline(s) {
  const parts = [];
  let rest = s;
  let key = 0;
  // Order matters: markdown links before bare URLs so we don't double-match the URL inside [..](..)
