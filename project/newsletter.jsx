...
const { useState: useStateNL, useMemo: useMemoNL } = React;

function CoverArt({ article }) {
  const { cover, coverPattern, category } = article;
  // Image cover: "image:<dataURL>"
...
function CoverPattern({ kind, dark }) {
...
function ArticleCard({ a, size, onOpen }) {
...
function NewsletterPage({ layout, articles, onAdd, onOpen }) {
  const { user, setLoginOpen } = useAuth();
  const [filter, setFilter] = useStateNL("All");
  const [composing, setComposing] = useStateNL(false);
...
Object.assign(window, { NewsletterPage, ArticleCard, CoverArt, brandFromInitials });
