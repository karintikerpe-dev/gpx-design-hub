...
function PromptsPage() {
  ...
  const [prompts, setPrompts] = useStatePL(() => {
    try {
      const stored = localStorage.getItem("gpx_prompts");
      if (stored) return JSON.parse(stored);
    } catch {}
    return window.GPX_DATA.PROMPTS;
  });
  React.useEffect(() => {
    try { localStorage.setItem("gpx_prompts", JSON.stringify(prompts)); }
    catch (err) { console.warn("Could not persist prompts:", err); }
  }, [prompts]);
  const [q, setQ] = useStatePL("");
  const [tool, setTool] = useStatePL("All");
  const [cat, setCat] = useStatePL("All");
  const [open, setOpen] = useStatePL(null);
  const [composing, setComposing] = useStatePL(false);
  const [copied, setCopied] = useStatePL(null);
