// Team page — 3 sections, member cards with edit-own-card

const { useState: useStateTM } = React;

function formatTenure(joinedDate, fallback) {
  if (!joinedDate) return fallback || "—";
  // Accept "YYYY-MM" or "YYYY-MM-DD" — parse to local-time day 1 for stable month math.
...
function TeamCard({ member, style, isMe, onEdit }) {
  const introExtra = member.introExtra; // 0 = full introvert, 100 = full extravert
  ...
  return (
    <article className={"team-card style-" + style + (isMe ? " is-me" : "")}>
      {isMe &&
      <button className="edit-pill" onClick={() => onEdit(member)}>✎ Edit my card</button>
      }
...
function PhotoArt({ seed }) {
  // Procedural avatar art — no real photos
  let h = 0;
...
function EditCardModal({ member, onClose, onSave }) {
  const [form, setForm] = useStateTM({ ...member, hobbies: member.hobbies.join(", ") });
...
function TeamPage({ cardStyle }) {
  const { user } = useAuth();
  const [team, setTeam] = useStateTM(() => {
    try {
      const stored = localStorage.getItem("gpx_team");
      if (stored) return JSON.parse(stored);
    } catch {}
    return window.GPX_DATA.TEAM;
  });
  React.useEffect(() => {
    try { localStorage.setItem("gpx_team", JSON.stringify(team)); }
